...
type ForegroundProps = Pick<SVGProps<SVGPathElement>, 'fill'> & { drsColor: string }

const Foreground = forwardRef<SVGPathElement, ForegroundProps>(
  ({ drsColor }, ref): JSX.Element => (
...
const getOffsetFactory = (maxRPM: number) => () => `${Math.max(1 - mutation.rpmTarget / maxRPM, 0).toFixed(2)}`
const getDRSColorFactory = () => () => mutation.drsAvailable ? (mutation.drsUsed ? '#C70000' : '#2CC767') : '#132237'

export const Gauge = () => {
  const ref = useRef<SVGStopElement>(null)
  const refCol = useRef<SVGPathElement>(null)
  const maxRPM = 15000 // useStore((state) => state.vehicleConfig.maxSpeed)

  const getOffset = getOffsetFactory(maxRPM)
  const getDRSColor = getDRSColorFactory()

  let offset = getOffset()
  let drsColor = getDRSColor()
  ...
  useEffect(() => {
    addEffect(() => {
      ...
      drsColor = getDRSColor()
      if (refCol.current.getAttribute('fill') !== drsColor) {
        refCol.current.setAttribute('fill', drsColor)
      }
    })
  })
  ...
  return (
    <div className="speed-gauge">
      ...
      <Foreground drsColor={drsColor} ref={refCol} />
    </div>
  )
}
